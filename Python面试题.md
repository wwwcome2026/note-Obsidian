...
原理是：选择相似度度量方法后通过一个向量模型将数据转化为向量。检索时，先进行索引加速，在执行查询。
相似度度量方法有：余弦相似度-适合文本，欧氏距离-适合图像，内积。

## 5. Langchain的基础组件有哪些? 
Models组件：提供了各种模型的集成，为所有模型提供了一个精简的统一接口，支持大模型、向量模型、聊天模型。
Prompts组件：主要用于构建提示词，方便提示词的动态书写
Chain组件：通过标准化接口将离散的组件（如提示词模版、大模型、外部工具等）串联成一个工作流，多用于处理那些需要多步骤协作的复杂任务。
