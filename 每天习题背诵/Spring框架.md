...
| 英文名称            | 中文翻译                    |
| :-------------- | :---------------------- |
| `required`      | 必须的（默认，如果当前有事务则加入，否则新建） |
| `requires_new`  | 新建独立事务（挂起当前事务）          |
| `supports`      | 支持（有则加入，无则非事务执行）        |
| `not_supported` | 不支持（以非事务方式执行，挂起当前事务）    |
| `mandatory`     | 强制的（必须运行在已有事务中，否则抛异常）   |
| `never`         | 绝不（必须不在事务中运行，否则抛异常）     |
| `nested`        | 嵌套（在当前事务中创建嵌套事务，利用保存点）  |
